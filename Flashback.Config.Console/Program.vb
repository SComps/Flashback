...
Module Program
    ...
    Private Sub DisplayHelp()
        ...
        Say("EMAIL ENABLED   : Set to YES to email PDFs after generation.", 2, 13, ConsoleColor.Cyan)
        Say("EMAIL RECIPIENTS: Comma-separated list of email addresses.", 2, 14, ConsoleColor.Cyan)
        Say("SMTP SERVER     : SMTP server hostname (e.g., smtp.gmail.com).", 2, 15, ConsoleColor.Cyan)
        Say("SMTP PORT       : SMTP port (typically 587 for TLS, 25 for plain).", 2, 16, ConsoleColor.Cyan)

        Say("COMMANDS:", 2, 18, ConsoleColor.Yellow)
        Say("ADD, SAVE, USERS, EXIT, [ID] to Edit, DELETE [ID], CONNECT [ID], UP, DOWN", 2, 20, ConsoleColor.Cyan)

        Say("Press any key to return...", 2, 22, ConsoleColor.White)
